...
import { sampleCircle } from "../utils"
import { Leg } from "./leg"
...
export class Hexapod extends EventTarget {
  ...
  calculateCoxaPoints(): Array<THREE.Vector3> {
    ...
    matrix.makeScale(config.hexapod.coxa.scale.x, config.hexapod.coxa.scale.y, config.hexapod.coxa.scale.z)
  ...
  updateMesh() {
    ...
    this.mesh.geometry = this.createExtrudeGeometry(shapePoints)
  }
   ...
   */
  updateBodyFK(pose: Pose, joints: Array<JointAngles>): boolean | Array<THREE.Vector3> {
    // console.log(joints);
    ...
    const success = this.updateBodyIK(pose, endpoints as Array<THREE.Vector3>)
