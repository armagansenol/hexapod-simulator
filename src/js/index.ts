...
class App {
   ...
   */
  updateAspectRatio() {
    const controlPanel = document.querySelector("main.control-panel") as HTMLElement
    const width = window.innerWidth
    const height = window.innerHeight - controlPanel.clientHeight
  ...
  createScene(): THREE.Scene {
    ...
    document.body.style.backgroundColor = `#${numberToHexString(config.colorBackground)}`

    // Variables commented out as they're currently unused
  ...
  createAmbientLight(scene: THREE.Scene): THREE.AmbientLight {
    const light = new THREE.AmbientLight(config.lights.ambient.color, config.lights.ambient.intensity)
  ...
  createDirectionalLight(scene: THREE.Scene): THREE.DirectionalLight {
    const light = new THREE.DirectionalLight(config.lights.directional.color, config.lights.directional.intensity)
    ...
    if (config.lights.directional.shadow.cast) {
      ...
      light.shadow.mapSize.width = config.lights.directional.shadow.mapSizeWidth
      light.shadow.mapSize.height = config.lights.directional.shadow.mapSizeHeight
   ...
   */
  createOrbitControls(camera: THREE.Camera, renderer: THREE.WebGLRenderer): OrbitControls {
